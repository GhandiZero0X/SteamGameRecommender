...
# print("Evaluation Results:", predictions[:5])  # Display the first 5 predictions for illustration
print("panjang predictions", len(predictions))
...
def collaborative_recommendation_with_details(user_id, model, data, games_df, top_n=10):
    ...
    for game, score in recommendations:
        ...
        if not game_details.empty:
            ...
            overall_rating = game_details['overall_player_rating'].values[0]
            detailed_recommendations.append((game, score, genre, overall_rating))
        else:
            detailed_recommendations.append((game, score, "Unknown", "Not Available"))
...
for game, score_rating, genre, overall_rating in detailed_recommendations:
    print(f"\nGame: {game}\nScore: {score_rating:.2f}\nGenre: {genre}\nOverall Rating: {overall_rating}")
